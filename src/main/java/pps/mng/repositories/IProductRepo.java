...
public interface IProductRepo extends JpaRepository<Product, Long> {
	...
	@Query("SELECT p FROM Product p WHERE p.name like ?1% AND p.active = 1") //Fijarse si necesita lista realmente o no
	List<Product> findByName(String name);
	
	@Query("SELECT p FROM Product p WHERE p.code like ?1 AND p.active = 1") //Fijarse si necesita lista realmente o no
	...
	@Query("SELECT p FROM Product p WHERE p.active = 1 ORDER BY p.code")
